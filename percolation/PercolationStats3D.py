...
class PercolationStats3D(object):
    ...
    def __init__(self, N, T):
        ...
        for x in xrange(T):
            perc = percolation3d.Percolation3D(N)
            thresh = 0
            while not perc.percolates():
                ...
                k = numpy.random.randint(1, N+1)
                while perc.is_open(i, j, k):
                    i = numpy.random.randint(1, N+1)
                    j = numpy.random.randint(1, N+1)
                    k = numpy.random.randint(1, N+1)
            ...
            self._results[x] = thresh / float(N**3)
